...
accCounter <- function(z,x,y) {
  count <- vector("numeric", length = length(z))
  for (i in 1:length(z)) {
    if (z[i] >= x) {count[i] = 1}
  }
  countDf <-data.frame("values" = rle(count)$values, "lengths" = rle(count)$lengths)
  countThreshold <- countDf[countDf$values == 1,]
  count0.5 <- countThreshold[countThreshold$lengths >= y,]
  return(sum(count0.5$values))
}

accCounter(player1$acceleration, 2, 20)
...
ggplot(data = player1,
 aes(x = seconds, y = angularVelocity, color = angularVelocity)) +
 geom_line() +
